...
public class SystemInitializer {
    public static void initializeSystem(SupplierSystemService service) {
        /* ---------------- Suppliers ---------------- */
        service.addSupplierWithDelivery("Walmart", "SUP001", "111222333", "1,3,5");
        service.addSupplierNeedsPickup("Amazon",  "SUP002", "444555666", "123 Tech Avenue");
        ...
        /* ---------------- Walmart agreement & order ---------------- */
        if (service.createAgreement("SUP001", 0, 2, LocalDate.now().minusDays(5), LocalDate.now().plusDays(30), List.of(0,1))) {
            System.out.println("🔍 Walmart agreement created with products [0,1]");
            Map<Integer,Integer> walmartItems = Map.of(0,3, 1,2); // WM001 x3, WM002 x2
            service.insertOrder("SUP001", LocalDate.now(), LocalDate.now().plusDays(7), walmartItems,
                    "John Doe", "0501234567", 0, 1); // agreement idx 0, IN_PROCESS
            System.out.println("✅ Walmart order initialized.");
        }

        /* ---------------- Amazon agreement & order ---------------- */
        if (service.createAgreement("SUP002", 1, 3, LocalDate.now().minusDays(10), LocalDate.now().plusDays(60), List.of(0,1))) {
            System.out.println("🔍 Amazon agreement created with products [0,1]");
            Map<Integer,Integer> amazonItems = Map.of(0,5); // AM001 x5
            service.insertOrder("SUP002", LocalDate.now(), LocalDate.now().plusDays(5), amazonItems,
                    "Jane Smith", "0509876543", 0, 0); // agreement idx 0, DELIVERED
            System.out.println("✅ Amazon order initialized.");
        }
