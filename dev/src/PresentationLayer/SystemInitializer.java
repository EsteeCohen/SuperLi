...
public class SystemInitializer {
    public static void initializeSystem(SupplierSystemService service) {
        ...
        /* ---------------- Products ---------------- */
        service.addProduct("P1" , "SUP001", "WM001", 10, new ArrayList<>(List.of("10,5%", "20,10%")), 25.0, 0);
        service.addProduct("P2","SUP001", "WM002",  5, new ArrayList<>(List.of("15,7%")),15.0, 1);
        service.addProduct("P3", "SUP002", "AM001",  8, new ArrayList<>(List.of("5,3%",  "10,6%")),30.0, 0);
        service.addProduct("P4", "SUP002", "AM002", 12, new ArrayList<>(),50.0, 1);
        System.out.println("Products initialized.");

        /* ---------------- Supplier 1 agreement & order ---------------- */
        if (service.createAgreement("SUP001", 0, 2, LocalDate.now().minusDays(5), LocalDate.now().plusDays(30), Set.of(0,1))) {
            ...
            service.insertOrder("SUP001", LocalDate.now(), LocalDate.now().plusDays(7), Supplier1Items,
                    "John Doe", "0501234567", 0, 2); // agreement idx 0, IN_PROCESS
            System.out.println("Supplier 1 order initialized.");
        }

        /* ---------------- Supplier 2 agreement & order ---------------- */
        if (service.createAgreement("SUP002", 1, 3, LocalDate.now().minusDays(10), LocalDate.now().plusDays(60), Set.of(0,1))) {
